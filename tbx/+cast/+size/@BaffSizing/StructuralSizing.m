...
for n = 1:opts.WingboxMaxStep+1
    ...
    Log.trace(sprintf('Structural Sizing %s, Step %.0f',obj.Name,n),'high');
    ...
    Log.trace(sprintf('Wingbox Sizing step %.0f Complete, Total Percentage Change %.2f',n,indicator*100),'high');
    
end
Par = Par{end};
obj.WingBoxParams = Par;

Log.debug(sprintf('Structural Sizing of %s Complete!',obj.Name),Length=60);
