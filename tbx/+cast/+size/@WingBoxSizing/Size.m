...
Log.trace('Sizing Wingboxes','mid');
for i = 1:length(obj)
    clear Par
    Par(1) = obj(i);
    for k = 1:opts.MaxStep
        ...
        Log.trace(sprintf('Sizing Wingbox %.0f, Substep %.0f, Max. Percentage Change %.2f',i,k,indicator*100),'low');
        if indicator*100 < opts.Converge
            Log.trace(sprintf('Wingbox %.0f Sizing Complete!',i),'mid');
            break
        else
            if k == opts.MaxStep
