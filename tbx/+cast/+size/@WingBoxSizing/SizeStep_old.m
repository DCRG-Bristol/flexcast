function Par = SizeStep_old(obj,Loads,SafetyFactor,opts)
arguments
...
% new code for web thickness update - ensures that thicknesses below the constraint are set to the constraint and only the free thickness is adjusted by the step size function
t_free = Par.SparWeb_Thickness - SparWeb_Thickness_Constraint;
t_free = max(t_free,0);
t_free = t_free.*SparWeb_adjust;
Par.SparWeb_Thickness = t_free + SparWeb_Thickness_Constraint;

%  old code for web thickness update
% if opts.optiSizing
